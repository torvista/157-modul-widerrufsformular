...
* @version $Id: widerrufsformular_builder.php 2026-03-07 15:39:40Z webchills $
*/
	require('includes/application_top.php');
	
	require(DIR_WS_CLASSES . 'widerrufsformular_builder.php');
	$highest_page_number ='';
	$cprObj = new widerrufsformular_builder();
	/* DATABASE OPERATIONS */
			...
			<table border="0" width="100%" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td class="pageHeading" align="right">
						<button type="button" class="btn btn-default btn-md" onClick="window.location='<?php echo zen_href_link(FILENAME_WIDERRUFSFORMULAR_BUILDER,'?action=addForm','NONSSL'); ?>'">
...
<?php
	if( !in_array($cprObj->GetAction(), array('addForm', 'editForm') ) ){ #IF NOT ADD/EDIT FORM
		...
		if( (int)WIDERRUFSFORMULAR_NUMBER_ROWS > 10 ){
			$number_of_rows = WIDERRUFSFORMULAR_NUMBER_ROWS; #CONFIGURATION
		}
		#GET PARAMETERS
		$parameters = zen_get_all_get_params(array('action','rID','page'));
		#LOAD DATA
		if( $sniffer->table_exists( TABLE_WIDERRUFSFORMULAR ) ){
...
?>
							<tr class="<?php echo $tr_class; ?>" onMouseOver="rowOverEffect(this)" onMouseOut="rowOutEffect(this)" onClick="document.location.href='<?php echo zen_href_link(FILENAME_WIDERRUFSFORMULAR_BUILDER, 'fID=' . $rec->fields['form_id']); ?>'" \>
								...
								<td class="dataTableContent"><?php echo date("d.m.Y", strtotime($rec->fields['timestamp'])); ?></td>
...
<?php
		...
		#BUTTONS
		if( $cprObj->GetAction() == 'addForm' ){
			$form .= zen_image_submit('button_widerrufsformular_add.gif', BTN_ADD) . "\n";
		}else{ #update
			...
			case 'addField':
				...
				$contents[] = array(
					'text' => zen_draw_label(
						...
						zen_draw_input_field(
							...
							$label, 
							zen_set_field_length(
								TABLE_WIDERRUFSFORMULAR_FIELDS, 
								'label',
								20
							) .
				...
				$contents[] = array(
					...
					'text' => zen_image_submit(
									'button_widerrufsformular_add.gif', 
									BTN_ADD,
						...
						if( $show['text'] ){
							$contents[] = array(
								...
								zen_draw_input_field(
									...
									'', 
									zen_set_field_length(
										TABLE_WIDERRUFSFORMULAR_FIELDS_OPTIONS, 
										'field_text',
										20
		...
		if( count($cprObj->GetFields()) > 0 ){
			...
			$contents[] = array(
					...
					zen_image_button(
						'button_widerrufsformular_add.gif', 
						BTN_ADD,
						'style="float:right;" onClick="window.location.href=\'' . zen_href_link(FILENAME_WIDERRUFSFORMULAR_BUILDER,'?action=addField&fID=' . $cprObj->GetFormId() . '','NONSSL') .'\'" class="btn"'
					)
				);
