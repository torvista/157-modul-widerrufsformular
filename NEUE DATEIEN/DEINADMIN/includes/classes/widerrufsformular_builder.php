...
*/
class widerrufsformular_builder{
	...
	private function Fields( $fID ){
		...
		if( $sniffer->table_exists( TABLE_WIDERRUFSFORMULAR_FIELDS ) ){
	...
	private function AvailableFieldTypes(){
		...
		if( $sniffer->table_exists( TABLE_WIDERRUFSFORMULAR_FIELDS ) ){
			$sql = "SELECT SUBSTRING(COLUMN_TYPE,5) AS type
					...
					AND TABLE_NAME = '" . TABLE_WIDERRUFSFORMULAR_FIELDS . "'
	...
	public function HitCount( $fID ){
		...
		if( $sniffer->table_exists( TABLE_WIDERRUFSFORMULAR_HITS ) ){
			$sql = "SELECT COUNT(*) AS count
					FROM `" . TABLE_WIDERRUFSFORMULAR_HITS . "`
					WHERE `form_id` = :formID";
			$sql = $db->BindVars($sql, ':formID', $fID, 'integer');
			$rec = $db->Execute( $sql );
			if( !$rec->EOF ){
	...
	public function GetForm( $action = '', $fID = 0, $ffID = 0, $oID = 0 ){
		...
		$form = zen_draw_form(
				...
				FILENAME_WIDERRUFSFORMULAR_BUILDER, 
	...
	public function GetWidget( $fID ){
		...
		$widget = '<div class="buttonRow forward"><a href="' . $url . '"><input class="cssButton submit_button button  button_send" onmouseover="this.className=\'cssButtonHover\'" onmouseout="this.className=\'cssButton submit_button button  button_send\'" type="button" value="' . WIDERRUFSFORMULAR_WIDGET_BUTTON_TEXT . '" /></a></div>';
	...
	public function GetWidgetLink( $fID ){
		...
		$url .= 'index.php?main_page=' . CATALOG_WIDERRUFSFORMULAR_MAIN_PAGE . '&form_id=' . $fID;
		return $url;
	}
	...
	public function InsertForm( $post ){
		...
		if( $error_message != '' ){
			...
			$insertID = 0;
			$sql = "INSERT INTO `" . TABLE_WIDERRUFSFORMULAR . "`
	...
	public function InsertFormField( $post ){
		...
		if( isset($post['fID']) and (int)$post['fID'] > 0 ){
			//INSERT FORM FIELD
			$sql = "INSERT INTO `" . TABLE_WIDERRUFSFORMULAR_FIELDS . "`
	...
	public function InsertOption( $post ){
		...
		if( isset($post['ffID']) and (int)$post['ffID'] > 0 ){
			//INSERT FORM FIELD
			$sql = "INSERT INTO `" . TABLE_WIDERRUFSFORMULAR_FIELDS_OPTIONS . "`
	...
	public function UpdateForm($post){
		...
		$success = false;
		if( isset($post['fID']) and (int)$post['fID'] > 0 ){
			$sql = "UPDATE `" . TABLE_WIDERRUFSFORMULAR . "`
	...
	public function UpdateFormField($post){
		...
		if( (int)$post['fID'] > 0 ){
			$field_name = $this->FieldNameValidator( $post['fID'], $post['txtName']);
			$sql = "UPDATE `" . TABLE_WIDERRUFSFORMULAR_FIELDS . "`
					SET
						`field_type` 	= :fType, 
						`field_name`	= :fName,
						`label`			= :fLabel,
	...
	public function UpdateOption($post){
		...
		$success = false;
		if( (int)$post['oID'] > 0 ){
			$sql = "UPDATE `" . TABLE_WIDERRUFSFORMULAR_FIELDS_OPTIONS . "`
					SET
						`field_text` 	= :fText, 
						`field_value`	= :fValue,
						`read_only`		= :fReadOnly,
	...
	public function DeleteForm($formID){
		...
		if( $formID > 0 ){
			...
			#DELETE FORM
			$sql = "DELETE FROM `" . TABLE_WIDERRUFSFORMULAR . "`
					WHERE `form_id` = :fID";
	...
	public function DeleteFormField( $fieldID ){
		...
		if( $fieldID > 0 ){
			...
			#DELETE FORM FIELD
			$sql = "DELETE FROM `" . TABLE_WIDERRUFSFORMULAR_FIELDS . "`
					WHERE `form_field_id` = :ffID";
			$sql = $db->BindVars($sql, ':ffID', $fieldID, 'integer');
			$success = $db->Execute($sql);
		}
	...
	public function DeleteOption( $oID ){
		...
		$success = false;
		if( (int)$oID > 0 ){
			$sql = "DELETE FROM `" . TABLE_WIDERRUFSFORMULAR_FIELDS_OPTIONS . "`
					WHERE `form_field_option_id` = :oID";
			$sql = $db->BindVars($sql, ':oID', $oID, 'integer');
			$success = $db->Execute($sql);
		}
